...
export default function TechCard({ tech, index }) {
  ...
  return (
    /* We use py-12 to ensure each row has its own clear space */
    <div className={`flex flex-col lg:flex-row items-center gap-10 lg:gap-16] py-12 lg:py-20 border-b border-gray-50 last:border-0 ${isReversed ? 'lg:flex-row-reverse' : ''}`}>
      
      {/* 🎥 VIDEO SIDE: 60% width */}
      <div className="w-full lg:w-[60%] relative group">
        {/* Architectural Frame - adjusted to be more subtle and not interfere with other cards */}
        <div className={`absolute -inset-2 border border-[#0056b3]/10 transition-all duration-700 hidden lg:block
          ${isReversed ? '-translate-x-4' : 'translate-x-4'} 
          translate-y-4 group-hover:translate-x-0 group-hover:translate-y-0`}>
        </div>

        <div className="relative z-10 aspect-video overflow-hidden rounded-sm bg-black shadow-lg">
          <video 
            src={tech.video} 
            autoPlay 
            loop 
          ...
          <div className="absolute top-4 left-4">
             <div className="px-3 py-1 bg-black/40 backdrop-blur-md border border-white/10 rounded-sm">
                <span className="text-[9px] text-white uppercase tracking-[0.3em] font-medium">Bosch Precision</span>
             </div>
          </div>
        </div>
      </div>
      ...
      <div className="w-full lg:w-[40%] flex flex-col justify-center text-center lg:text-left">
        <div className="mb-4">
          {/* Removed the absolute "large" number that was causing the overlap issues */}
          <div className="flex items-center justify-center lg:justify-start gap-3 mb-2">
            <span className="text-[#0056b3] font-mono text-sm font-bold tracking-tighter">0{tech.id}</span>
            <div className="w-8 h-px bg-[#0056b3]/30"></div>
            <p className="text-[#0056b3] font-bold text-[10px] uppercase tracking-[0.4em]">
              {tech.tagline}
            </p>
          </div>
          
          <h2 className="text-4xl md:text-5xl font-serif italic tracking-tight text-[#1a1a1a] leading-tight">
            {tech.title}
          </h2>
        </div>

        <p className="text-gray-500 font-light leading-relaxed text-base mb-8 max-w-md mx-auto lg:mx-0">
          {tech.desc}
        </p>
        ...
        <div>
          <Link 
            to="/contact-visit" 
            className="inline-flex items-center gap-4 px-6 py-3 bg-[#000d1a] text-white text-[10px] uppercase tracking-[0.3em] font-bold hover:bg-[#0056b3] transition-all duration-300 rounded-sm"
          >
